...
public String compressLine(String line, char[] fileChars){
   //TODO: Implement this method
      ...
      for (int i = 0; i < line.length() - 1; i ++)
      {
        ...
        {
          compress = compress + sequence + "," ;
          sequence = 1; 
        }
      }
      return compress + sequence;
}
  ...
  public String[] compressLines(String[] lines, int dataSize, char[] fileChars){
      //TODO: Implement this method
      String[] compressLines = new String [dataSize];
      for (int i = 0; i < dataSize; i++)
      {
        compressLines[i] = compressLine(lines[i], fileChars);
      }
...
public String getCompressedFileStr(String[] compressed, char[] fileChars) {
    //TODO: Implement this method
      ...
      compressedFileStr = compressedFileStr + fileChars[0] + "," + fileChars[1] + "\n";
      for (int i = 0; i < compressed.length; i++)
      {
        compressedFileStr = compressedFileStr + compressed[i] + "\n";
      }
   ...
   */
   public String decompressLine(String line, char[] fileChars){
      //TODO: Implement this method
      String decompress = "";
      String[] lineNocomma = line.split(",");
      if (lineNocomma[0] == "0"){
        for (int i = 1; i < lineNocomma.length; i++)
        {
          int lineInt = Integer.parseInt(lineNocomma[i]);
          for (int j=0; j<lineInt; j++){
          decompress = decompress + fileChars[i%2 + 1];
          }
        }
      }
      else {
        for (int i = 0; i < lineNocomma.length; i++)
        {
          int lineInt = Integer.parseInt(lineNocomma[i]);
          for (int j=0; j<lineInt; j++){
          decompress = decompress + fileChars[i%2 + 0];
          }
        }
      }
        return decompress;
  ...
  public String[] decompressLines(String[] lines, int dataSize){
     //TODO: Implement this method
      String[] decompressLines = new String [dataSize];
      char[] fileChars = new char[10];
      fileChars[0] = lines[0].charAt(0);
      fileChars[1] = lines[0].charAt(2);
      for (int i = 0; i < dataSize-1; i++)
      {
        decompressLines[i] = decompressLine(lines[i+1], fileChars);
      }
...
//TODO: Implement this method
  ...
  for (int i = 0; i < dataSize; i++){
    String line = decompressed[i];
    fileChars[0] = line.charAt(0);
    for (int j = 0; j < line.length(); j++){
      if(line.charAt(j) != fileChars[0]){
        fileChars[1] = line.charAt(j);
        break;
      }
    }
  }
